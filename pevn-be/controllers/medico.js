...
medico.readNascimento = async (req, res) => {
    const id = req.params.id_c;
    try {
        const nascimento = await (await pool.query('SELECT * FROM rn_nascimentos WHERE nseq=$1', [nseq])).rows[0];
        res.status(200).json({nascimento});
    } catch (error) {
        res.status(500).json({
